...
def eval_schaetzfrage_bahnhof(update, context):
    ...
    if schaetzung == echter_wert:
        update.message.reply_text('Not bad! You nailed it 😉',
                                  reply_markup=ReplyKeyboardRemove())
    elif schaetzung >= echter_wert-echter_wert*0.2 and schaetzung <= echter_wert+echter_wert*0.2:
        update.message.reply_text('Very close!',
                                  reply_markup=ReplyKeyboardRemove())
    else:
        update.message.reply_text("No, that's not quite right!",
                                  reply_markup=ReplyKeyboardRemove())
...
def eval_schaetzfrage_reiherberg(update, context):
    ...
    if schaetzung == echter_wert:
        ...
        update.message.reply_text('You are close!',
                                  reply_markup=ReplyKeyboardRemove())
    else:
        update.message.reply_text('No, not even close 😬',
                                  reply_markup=ReplyKeyboardRemove())
...
def eval_kirche_wortraetsel(update, context):
    ...
    if re.sub('\W+', '', antwort.lower()) == re.sub('\W+', '', echter_wert.lower()):
        ...
        update.message.reply_text('You are close!', reply_markup=ReplyKeyboardRemove())
...
def eval_storchenbank(update, context):
    ...
    if antwort == echter_wert:
        update.message.reply_text('So you have discovered the board! There, the return times and the offspring of the stork pair are recorded for many years.',
                                  reply_markup=ReplyKeyboardRemove())
    else:
        update.message.reply_text(
            'Almost! Next to the stork bench you will find a board on which the return times and the offspring of the stork pair have been recorded for many years.', reply_markup=ReplyKeyboardRemove())
